var Tree = new function() {
    ...
    this.getChildren = function(_parentId) {
        ...
        if (childBox) {
            ...
            Tree.addTip(_parentId);
            var xmlHttp=this.createXmlHttp();                       //创建XmlHttpRequest对象
            xmlHttp.onreadystatechange = function() {
                if (xmlHttp.readyState == 4) {
                    Tree.removeTip(_parentId);
                    //调用addChildren函数生成子节点
                    Tree.addChildren(_parentId, xmlHttp.responseXML);
                }
            }
            xmlHttp.open("GET", this._url + "?browseNodeId=" + _parentId, true);
            xmlHttp.send(null);
        }
    }

    //显示加载状态提示信息
    this.addTip = function(_parentId) {
        var _parentNode = document.getElementById(_parentId);
        var _tipBox = document.createElement("span");
        _tipBox.className = "tip";
        _tipBox.appendChild(document.createTextNode(this._tipInfo));
        _parentNode.appendChild(_tipBox);
    }

    //取消加载状态提示信息
    this.removeTip = function(_parentId) {
        var _parentNode = document.getElementById(_parentId);
        _parentNode.removeChild(_parentNode.lastChild);
    }
    ...
    //创建节点的页面片断
    this.createItemHTML = function(itemId, itemType, itemData) {
        ...
        if (itemType == this._itemType) {
            ...
            return '<span class="itemMark">' + this._itemMark + '</span>' +
                   '<span class="item" onclick="Tree.clickItem();">' + itemData + '</span>';
